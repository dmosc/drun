...
#[mcp_tool(
    name = "session_diff",
    description = "Compute a unified diff between two checkpoints of the active session. Defaults to comparing the current checkpoint against the one right before it. Pass an explicit from_checkpoint_id (e.g. 0) to diff against something further back. Returns standard unified diff output across all changed files. Each endpoint accepts an ID or a label; label takes precedence. Pass paths to restrict the diff to specific files instead of every changed file.",
    idempotent_hint = true,
    destructive_hint = false,
    read_only_hint = true
)]
