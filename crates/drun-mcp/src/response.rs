...
impl ResponseBuilder {
    pub(crate) fn text(s: impl Into<String>) -> CallToolResult {
        CallToolResult::text_content(vec![TextContent::from(s.into())])
    }

    pub(crate) fn json(value: &impl Serialize) -> CallToolResult {
        Self::text(serde_json::to_string(value).unwrap_or_else(|_| "null".into()))
    }
    ...
    pub(crate) fn file_content(path: &str, bytes: &[u8]) -> CallToolResult {
        match Self::mime_type_for_extension(path) {
            Some(mime_type) => {
                let image =
            ...
            None => match std::str::from_utf8(bytes) {
                Ok(s) => Self::text(s),
                Err(_) => Self::text(format!("[Unknown format] {}", STANDARD.encode(bytes))),
            },
        }
    }
}
...
mod tests {
    ...
    fn mime_type_for_extension_recognizes_known_image_types() {
        assert_eq!(
            ResponseBuilder::mime_type_for_extension("photo.png"),
            Some("image/png")
        );
        assert_eq!(
            ResponseBuilder::mime_type_for_extension("photo.jpg"),
            Some("image/jpeg")
        );
        assert_eq!(
            ResponseBuilder::mime_type_for_extension("photo.jpeg"),
            Some("image/jpeg")
        );
        assert_eq!(
            ResponseBuilder::mime_type_for_extension("doc.pdf"),
    ...
    fn mime_type_for_extension_returns_none_for_unknown_extensions() {
        assert_eq!(ResponseBuilder::mime_type_for_extension("notes.txt"), None);
        assert_eq!(ResponseBuilder::mime_type_for_extension("main.rs"), None);
    }

    #[test]
    fn mime_type_for_extension_returns_none_for_a_path_with_no_extension() {
        assert_eq!(ResponseBuilder::mime_type_for_extension("Makefile"), None);
    }

    #[test]
    fn file_content_wraps_image_bytes_as_base64_image_content() {
        let result = ResponseBuilder::file_content("photo.png", b"fake-png-bytes");
    ...
    fn file_content_returns_plain_text_for_utf8_non_image_files() {
        let result = ResponseBuilder::file_content("notes.txt", b"hello world");
        match &result.content[0] {
    ...
    fn file_content_base64_encodes_non_utf8_non_image_files() {
        ...
        let result = ResponseBuilder::file_content("data.bin", &bytes);
        match &result.content[0] {
