...
step1 = session.execute_python("print(open('/workspace/state.txt').read())")
print(f"[checkpoint {step1.id}] {step1.stdout}")

session.execute_python(
    "with open('/workspace/state.txt', 'w') as f: f.write('step 2')")
step2 = session.execute_python("print(open('/workspace/state.txt').read())")
print(f"[checkpoint {step2.id}] {step2.stdout}")

session.rollback(step1.id)

after_rollback = session.execute_python(
    "print(open('/workspace/state.txt').read())")
print(
